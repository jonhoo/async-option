...
//! will return `Async::NotReady` if the `Option` is occupied or empty respectively. `Aption<T>`
//! can also be used as a `Sink<SinkItem = T>` and `Stream<Item = T>` by effectively operating as a
//! single-element channel.
//!
